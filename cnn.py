...
class Attack():
    def __init__(self):
        ...
        self.input_length = tf.placeholder(tf.int32, name='length')
        self.target_phrase = tf.placeholder(tf.string, name='phrase', shape=list)
        self.target_in = tf.placeholder(tf.int32, name='index', shape=list)
        self.target_length = tf.placeholder(tf.int32, name='len')
        self.arg = tf.placeholder(tf.float32, name='arg')
    ...
    def run_attack(self, sess, input_wavefile, output_wave_file, phrase):
        input_audio = load_wav(input_wav_file=input_wavefile)
        input_length = len(input_audio)
        weird = (self.target_length - 1) // 320

        target_in = [[toks.index(x) for x in phrase]]
        target_length = [len(phrase)]

        ctcloss = sess.run(self.ctc_loss, feedict={self.input: input_audio, self.input_length: input_length,
...
if '__name__' == '__main__':
    ...
    with tf.Session() as sess:
        ...
        attack.run_attack(sess, inp_wav_file, out_wav_file, phrase)
